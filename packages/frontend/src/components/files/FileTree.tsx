...
/** Upload files to a directory via drag & drop */
async function uploadFilesToDir(targetDir: string, e: React.DragEvent, onRefreshTree?: () => void) {
  // Skip internal drags
  ...
  try {
    ...
    if (ok.length > 0) toastSuccess(`${ok.length} file(s) uploaded to ${targetDir.split('/').pop()}/`);
    if (fail.length > 0) toastError(`${fail.length} file(s) failed: ${fail.map((f: any) => f.error).join(', ')}`);
    if (ok.length > 0) onRefreshTree?.();
  } catch (err) {
    console.error('[filetree] File upload error:', err);
  ...
  const handleContextAction = async (action: MenuAction, entry: FileEntry) => {
    ...
    if (action === 'download') {
      try {
        ...
        const headers: Record<string, string> = {};
        if (token) headers['Authorization'] = `Bearer ${token}`;
        const res = await fetch(`/api/files/serve?path=${encodeURIComponent(entry.path)}&download=1`, { headers });
